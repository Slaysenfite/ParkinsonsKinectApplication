    ...
    public class KNearestNeighbour
    {
        ...
        public static int Classify(double[] unknown, List<Iris> trainData, int numClasses, int k)
        {
            ...
            for (int i = 0; i < k; ++i)
            {
                ...
                output += "\n( " + trainData.ElementAt(info[i].i).getPLength() + ", "
                          + trainData.ElementAt(info[i].i).getSWidth() + ", "
                          + trainData.ElementAt(info[i].i).getPLength() + ", "
                          + trainData.ElementAt(info[i].i).getPWidth() + " ) : "
                            + dist + "        " + c;
            }
        ...
        static int Vote(IndexAndDistance[] info, List<Iris> trainData, int numClasses, int k)
        {
            ...
            for (int j = 0; j < k; ++j)  // just first k nearest
            {
                ...
                Boolean b = trainData.ElementAt(j).getIClass().Equals("Iris-virginica");
